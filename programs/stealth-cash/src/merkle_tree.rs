...
pub const ROOT_HISTORY_SIZE: u128 = 30;

#[derive(Debug, Clone, BorshDeserialize, BorshSerialize)]
pub struct MerkleTree {
    levels: u128,
    filled_subtrees: HashMap<u128, Vec<u8>>,
    roots: HashMap<u128, Vec<u8>>,
    current_root_index: u128,
    next_index: u128
}

impl MerkleTree {
    pub fn new(levels: u128) -> Self {
    ...
    pub fn hash_left_right(&self, left: Vec<u8>, right: Vec<u8>) -> Vec<u8> {
        #[allow(non_snake_case)]
        let mut R: u128 = u128::from_be_bytes(convert_to_array(&left));

        #[allow(non_snake_case)]
        let mut C: u128 = u128::from_be_bytes(convert_to_array(&right));
    
        let (r1, c1) = MiMCSponge(R, C);
        R = r1;
        C = c1;
    
        R = R.wrapping_add(u128::from_be_bytes(convert_to_array(&right)));
    
        let (r2, c2) = MiMCSponge(R, C);
        R = r2;
    
        let result_bytes = R.to_be_bytes();
    ...
    pub fn insert(&mut self, leaf: Vec<u8>) -> Result<u128> {
        if self.next_index < 2_u128.pow(self.levels as u32) {
            let e = AnchorError {
        ...
        for i in 0..self.levels {
            if current_index % 2 == 0 {
                ...
                left = self.filled_subtrees.get(&i).unwrap().to_vec();
    ...
    pub fn is_known_root(&self, root: Vec<u8>) -> bool {
        if root.is_empty() {
            return false;
        }
...
#[allow(non_snake_case)]
fn MiMCSponge(in_xL: u128, in_xR: u128) -> (u128, u128) {
    (in_xL, in_xR)
}
